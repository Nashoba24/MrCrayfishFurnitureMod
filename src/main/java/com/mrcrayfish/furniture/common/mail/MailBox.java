...
public class MailBox implements INBTSerializable<CompoundNBT>
{
    ...
    public CompoundNBT serializeNBT()
    {
        ...
        compound.putString("World", this.world.toString());
    ...
    public void deserializeNBT(CompoundNBT compound)
    {
        ...
        if (compound.contains("Dimension")) { //TODO doesn't work. 1.15 mailbox are lost while upgrading a world to 1.16
            int dimId = compound.getInt("Dimension");
            if (dimId == 1) {
                this.world = World.field_234918_g_;
            }
            else if (dimId == 0) {
                this.world = World.field_234919_h_;
            }
            else if (dimId == 2) {
                this.world = World.field_234920_i_;
            }
        }
        for (World w : ServerLifecycleHooks.getCurrentServer().getWorlds()) {
            if (w.func_234923_W_().toString().equals(compound.getString("World"))) {
                this.world = w.func_234923_W_();
                break;
            }
        }
